...
## TODO:
1. Break apart PCA plotting code in stepwise fashion
2. **Add subheadings** to PCA & DFA sections
3. In DFA section, add explanation of posterior probabilities (make a table?)
    + Really want some means of comparing *a priori* classification to DFA
4. Make DFA graph simpler
5. Add references
...
legend.cols <- c("black", "forestgreen", "cadetblue", "red3") 
...
In this second principal component, as mentioned before, m1 and m4 don't contribute much to the component [the magnitudes of their respective loadings (.173 and .031) are small compared to the other four skull measurements]. The remaining four loadings indicate a shape difference, with one pair of variables having positive loadings and one pair having negative loadings. This component describes variation in the four variables: samples with high values of m6 and m3 have low values of m2 and m5 and vice versa. You can confirm this looking again at a boxplot for these values, focusing only on those four variables of interest for the two species that are differentiated along PC2, _E. lutris_ and _P. brasiliensis_:
...
# Plot each of the four measurements in a different plot
#boxplot(formula = m2 ~ species, data = two.species, main = "m2", cex = 0.8)
#boxplot(formula = m3 ~ species, data = two.species, main = "m3", cex = 0.8)
#boxplot(formula = m5 ~ species, data = two.species, main = "m5", cex = 0.8)
#boxplot(formula = m6 ~ species, data = two.species, main = "m6", cex = 0.8)
...
### A brief interpretion of DFA results
...
+ demonstrate apply to pull out only rows with all FALSE
+ Extract the row indices for those ^^
+ show posterior probabilities for those rows
