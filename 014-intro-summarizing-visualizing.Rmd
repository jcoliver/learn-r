...
What if you wanted means and standard deviations for each species? You could copy and paste the code, changing the variable name and species name for every species, but since there are `r unique(subset_data$Common_Name)` different common names in these data, that would be tedious and a great way to make a mistake. Thankfully, there are additional packages in R that make this much easier.
...
We are going to use the [dplyr](https://dplyr.tidyverse.org/) package for summarizing our data. To install the dplyr:
...
Here we use the `group_by` function to essentially create a separate pile of data for each species, then for each "pile" calculate the mean and standard deviation:
