...
4. Crop bioclim SpatRasters to extent (+25%?) of saguaro data
5. Create background points (from one of those layers in the bioclim data)
6. Merge saguaro and background points (and add pa column)
7. Extract climate data for those saguaro and background points
8. Run glm
9. Predict probability values
10 Calculate thresholds and plot results
...
Next, there are _two_ additional R packages that will need to be installed:
...
# We want to sample an area slightly larger than observations' extent, so we 
# make an extent object that is 25% larger than our observations' extent
