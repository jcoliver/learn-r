...
<!--
Additional material to develop, if warranted
+ Back to dplyr & readr
...
iris %>% 
  ...
  ggplot(mapping = aes(x = Species, y = means)) +
  geom_point() +
...
+ The specification of what to plot on the y-axis in the `ggplot` function
    + In `ggplot` command: change `SL.mean` to `means`
+ The values for error bar boundaries
    + In `geom_errorbar` command: replace `SL.mean` with `means` and `SL.se` with `se`
