...
Hmmmm...that's an odd warning. Take a look at the elements of the object:

```{r}
ls(airquality.models)
```
...
Now when we run `RegressSimple`, there are only three elements in the list, as we expect:
...
RegressSimple <- function(data, response) {
  ...
  for (predictor in predictors) { # Run this once with just cat("Predictor: ", predictor, "\n")
    simple <- lm(airquality[, response] ~ airquality[, predictor])
    simple.summary <- summary(simple)
