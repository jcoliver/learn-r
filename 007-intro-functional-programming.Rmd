...
Want to standardize so each has mean zero and variance of one

New script file, iris-stand.R
```{r petal-length-standardization}
...
Repeat for petal width, copy/paste from above
```{r petal-width-standardization}
# Standardize petal length
mean.petal.width <- mean(iris$Petal.Width)
sd.petal.width <- sd(iris$Petal.Width)
standard.petal.width <- (iris$Petal.Width - mean.petal.width)/sd.petal.width
summary(standard.petal.width)
```

Did copy/paste really save time? And how many opportunities for mistakes?
