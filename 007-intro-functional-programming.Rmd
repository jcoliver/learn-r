...
```{r function-documentation-overview, eval = FALSE}
# goal: calculate mean, sd, and transform to mean = 0, var = 1
# input: one column of numeric data
# output: standardized variables
```
...
```{r function-documentation-input, eval = FALSE}
# goal: calculate mean, sd, and transform to mean = 0, var = 1
# data: one column of numeric data
# output: standardized variables
```

Next, we focus on the output. In the R language, we generally refer to this as the data that the function _returns_ (remember that we used the `return` command at the end of our function). In this case, we want to replace "output" with "returns" and add a little more detail about the data that are produced by this function.

```{r function-documentation-output, eval = FALSE}
# goal: calculate mean, sd, and transform to mean = 0, var = 1
# data: one column of numeric data
...
```{r function-documentation-goal, eval = FALSE}
...
If you want to use functions in _other_ R code you write, the best thing to do is to remove the function code from your script and to put it in a separate file. Here we are going to create a new folder, just called "R", and move the code for the `standardize` function to a new file. We generally put a single function in a file, so we'll make a new file called "standardize.R" and save it in that R folder. 

The contents of standarize.R:

```{r function-separate-file, eval = FALSE}
# Standardize a vector of numerical data
# data: one column of numeric data
# returns: vector of numeric values, transformed to have mean of zero and 
#          variance of 1
...
However, now we need to explicitly load this function into memory in our script, since the code for the function is no longer there. We use the command `source` to load the function into memory in our script.

```{r example-use-source, eval = FALSE}
# Standardize iris values
# Jeff Oliver
# jcoliver@email.arizona.edu
# 2018-08-16

source(file = "R/standardize.R")
...
The `standardize` function allows us to tranform a single column of numeric data to have a mean of 0 and variance of 1. But what if we wanted to do that for _all_ the columns of a data frame? How could we do that?


If we wanted to write code to standardize all the relevant columns of data, we could do it piece by piece, one line for each column:

```{r all-columns, eval = FALSE}
standard.petal.length <- standardize(data = iris$Petal.Length)
standard.petal.width <- standardize(data = iris$Petal.Width)
standard.sepal.length <- standardize(data = iris$Sepal.Length)
standard.sepal.width <- standardize(data = iris$Sepal.Width)
```

Which may not be too big a deal with only four columns. If there are more that a few columns, though, that gets tedious and once again introduces the problems that can arise with multiple copy-paste cycles.

### `apply` a function

If we want to perform the same operation over multiple columns, we can use the `apply` function. Here we are going to make an entirely new data frame, based on the iris data. But we will use the `standardize` function to transform the data in each column. To use `apply`, we provide three pieces of information:

+ `X`: The data frame to use
+ `MARGIN`: Whether we want to apply the function over rows (`MARGIN = 1`) or over columns (`MARGIN = 2`)
+ `FUN`: The name of the function to use

For our situation, this looks like:

```{r apply-function-01, error = TRUE}
iris.stand <- apply(X = iris, MARGIN = 2, FUN = standardize)
```

Hmmm...That's not good. The error message tells us that when calculating the `mean.value`, R encountered a non-numeric piece of data. Because a mean (and standard deviation, too) is only meaningful for numeric data, R stopped processing our request. Look again at the `iris` data frame with `head`:

```{r investigate-error}
head(iris)
```

The last column, `Species`, does indeed have non-numeric data, so we only want to use the function on the first four columns, and check our work with `summary`:
...
Note that the `Species` column is gone. When we passed the first four columns of data in our call to `apply`, the `Species` column got left out. We can add it back with:
...
Could also instead use `is.numeric` to _only_ tranform numeric columns.
