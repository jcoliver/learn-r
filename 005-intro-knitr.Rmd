...
And the last thing to mention about formatting is that if you want to include equations, you can use Tex syntax, surrounded by dollar signs (\$). Use single dollar signs for in-line equations, `$E = mc^2$` is rendered as $E = mc^2$. Equations in double dollar signs are displayed on their own line, so `$$E = mc^2$$` shows up as $$E = mc^2$$

And if you know LaTex, you can use that, too (remembering to bracket your LaTeX code with double dollar signs):
