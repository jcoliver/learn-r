...
[BITLY LINK](BITLY LINK) and [learn-rlink](learn-rlink) (the former just 
re-directs to the latter) and save it in the "data" folder we created at the 
start of the lesson. Save the file as "swift-new.csv" (it should be named 
this when you download it, but it is always good to double-check). We then load
the data in with `read.csv()`:
...
`full_model` we just estimated. Note that the first column has the track name, 
which is not part of the model (although it could be), so we exclude that 
column when making predictions by using the negation symbol:
...
[neural networks](https://www.youtube.com/watch?v=aircAruvnKk). This video 
does a good job breaking down how neural networks work to make predictions.
