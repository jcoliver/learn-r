...
If the install does not work, i.e. you see an warning message like:
...
you may need to install additional software. The [installation page](https://rspatial.github.io/terra/#installation) 
for the terra package has additional information. For Linux users, you will 
probably need to install the libgdal-dev package through the terminal, _then_ 
install terra with `install.packages()` in R.
...
```{r reverse-colors}
temp_cols <- hcl.colors(n = 50, palette = "YlOrBr", rev = TRUE)
