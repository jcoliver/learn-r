...
We need to make sure the land cover data are in a format we can use. Since the
land cover data is categorical, we need to convert the data from a integer to 
a factor.
...
For training and test the model, we split the data into a training set, which 
we used to build the model, and a testing set, which we use to evaluate the 
model. We will use a 80:20 training:testing split, which is common for SDM 
applications.
