...
distribution of R. Those packages are `ggplot2`, `ggdendro`, `reshape2`, and 
...
"species", "museum", and "accession" columns), but now has "variable" and 
...
stored in the "variable" column.  

We use the `reshape2` package's `melt` function to handle the data 
transformation:
...
Now we can use that new data frame, `otter.long` to create our heatmap. In the
...
heatmap. The order of those tips are stored in the `otter.dendro` object, and 
...
the long-format, `otter.long` object, and the column we used for the y-axis is
`accession`. Using the order of the dendrogram tips (we stored this above in 
the `otter.order` vector), we can re-level the `otter.long$accession` column to
...
calling `print` on `dendro.plot`, it may work better on your screen to set 
