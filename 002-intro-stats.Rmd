...
`data.frame` with the `dim` command. `dim(iris)` prints out the number of rows 
(`r nrow(iris)`) and the number of columns (`r ncol(iris)`): 
...
the ANOVA; we only have to enter the name of the variable to see the results.
...
So we reject the hypothesis that these species have the same petal lengths. The 
last thing we want to do with this code is save our results to a file. To do 
so, we put the call to `summary` between a pair of calls to `sink`:

```{r write-t-test, eval = FALSE}
sink(file = "output/t-test.txt")
setosa.v.versicolor
sink()
```

Notice now that because we have directed output to the file "t-test.txt", the 
output of `summary` will _not_ be output to the console. Open the file to make 
sure the output was saved correctly.  
...
As before, if we want to save these results to a file, we put the call to 
`summary` between a pair of calls to `sink`:
...
`r round(x = lifeExp.sum$coefficients['logGDP', 'Estimate'], digits = 3)`, 
...
The _F_-statistic = `r round(x = f.stat, digits = 2)`, and the p-value is quite 
...
relationship is 
`r round(x = lifeExp.v.gdp$coefficients['log2GDP'], digits = 2)`. Remember that 
...
increase of 
`r round(x = (2 * lifeExp.v.gdp$coefficients['log2GDP']), digits = 2)` years in 
life expectancy.
