...
column when making predictions by using the negation symbol. That is, for the 
`newdata` argument, we pass `new_album[, -1]`, which tells R to skip the first 
column:
...
# Predict chart position for each of the songs on the new album, skipping the 
...
+ A [PDF version](https://jcoliver.github.io/learn-r/018-ml-regression.pdf) of 
this lesson.
