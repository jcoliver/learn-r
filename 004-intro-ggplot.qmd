...
object, and finally printing the plot by running the line of code with just the 
name of the variable in which our plot is stored:
...
In the spirit of reproducibility, we would like this script to be able to run 
and _not_ depend on variables being loaded from other scripts. So, we want to 
start by loading the those gapminder data. To create a violin plot, we use the 
same approach as for a boxplot, but instead of `geom_point`, we use 
`geom_violin`:
