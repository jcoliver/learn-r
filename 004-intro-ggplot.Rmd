...
And yet there is still that empty space on the right-hand side of the graphs. Why? It's because we created a new data frame, called `gapminder.no.oceania`, but we did not refer to it in our `ggplot` call. Rather we were still referring to the original `gapminder` data, which still contains the Oceania data. So we need to update the `data` we pass to `ggplot`. We can also take out "violet" as the final color option, since we don't need it for Oceania in this plot:
...
# Create violin plot object
gdp.plot <- ggplot(data = gapminder.no.oceania, aes(x = continent, y = gdpPercap, fill = continent)) +
  geom_violin() +
  scale_y_log10() +
  ylab("GDP per capita") +
  theme_bw() +
